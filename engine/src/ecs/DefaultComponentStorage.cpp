...
template <typename T> void DefaultComponentStorage<T>::add(Entity entity, const T &component) noexcept {
...
template <typename T> T *DefaultComponentStorage<T>::get(Entity entity) noexcept {
...
template <typename T> void DefaultComponentStorage<T>::remove(Entity entity) noexcept {
  auto it = entity2index.find(entity);
  if (it == entity2index.end()) {
    return;
  }
